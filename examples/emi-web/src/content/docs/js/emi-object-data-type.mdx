...
export class MyObjectClassDto {
  ...
  static Profile = class Profile {
     ...
     **/
    toJSON(): MyObjectClassDtoType.ProfileType {
      return {
        firstName: this.#toPlainJSON(this.#firstName),
        age: this.#toPlainJSON(this.#age),
      } as MyObjectClassDtoType.ProfileType;
    }
    /**
   ...
   *	class gets, rather than a shallow object still holding class instances.
   **/
  toJSON(): MyObjectClassDtoType {
    return {
      profile: this.#toPlainJSON(this.#profile),
    } as MyObjectClassDtoType;
  }
  /**
   * Resolves value into a plain, JSON-serializable value: recurses into
   * arrays, and - the case JSON.stringify's own recursion would only get to
...
export class MyObjectClassDto {
  ...
  static OptionalProfile = class OptionalProfile {
     ...
     *	class gets, rather than a shallow object still holding class instances.
     **/
    toJSON(): MyObjectClassDtoType.OptionalProfileType {
      return {
        lastName: this.#toPlainJSON(this.#lastName),
      } as MyObjectClassDtoType.OptionalProfileType;
    }
    /**
     * Resolves value into a plain, JSON-serializable value: recurses into
     * arrays, and - the case JSON.stringify's own recursion would only get to
  ...
  toJSON(): MyObjectClassDtoType {
    return {
      optionalProfile: this.#toPlainJSON(this.#optionalProfile),
    } as MyObjectClassDtoType;
  }
  /**
   * Resolves value into a plain, JSON-serializable value: recurses into
