 ...
 */
// WebSocket upgrader
var upgraderSubscribeYokeAction = websocket.Upgrader{
	CheckOrigin: func(r *http.Request) bool { return true },
}

func (x *SubscribeYokeActionMessage) Connection() *websocket.Conn {
	return x.Conn.(*websocket.Conn)
}
func (x *SubscribeYokeActionClientSession) Connection() *websocket.Conn {
	return x.Socket.(*websocket.Conn)
}
func (x *SubscribeYokeActionSession) GinCtx() *gin.Context {
	return x.Ctx.(*gin.Context)
}
func (x *SubscribeYokeActionSession) GetSocket() *websocket.Conn {
	return x.Socket.(*websocket.Conn)
}

// Generated handler
func SubscribeYokeActionGin(r *gin.Engine, factory func(
	session SubscribeYokeActionSession,
) (chan []byte, error)) {
	meta := SubscribeYokeActionMeta()
	r.GET(meta.URL, SubscribeYokeActionReactiveHandler(factory))
}
func SubscribeYokeActionReactiveHandler(factory func(
	session SubscribeYokeActionSession,
) (chan []byte, error)) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		read := make(chan SubscribeYokeActionReadChan)
		done := make(chan bool)
		c, err := upgraderSubscribeYokeAction.Upgrade(ctx.Writer, ctx.Request, nil)
		if err != nil {
		...
		session.QueryParams = SubscribeYokeActionQueryFromHttp(ctx.Request)
		...
		go func() {
			for {
				...
				read <- SubscribeYokeActionReadChan{
					Data:  data,
					Error: err,
				}
				if err != nil {
...
// SubscribeYokeActionClientOptions configures a client dial. All fields are
...
type SubscribeYokeActionClientOptions struct {
	Query     url.Values
	TLSConfig *tls.Config
	Headers   http.Header
}

// SubscribeYokeActionClient dials the SubscribeYokeAction endpoint at baseURL
...
//
//	sess, err := SubscribeYokeActionClient(
//	    "wss://hub.example.com",
//	    SubscribeYokeActionClientOptions{TLSConfig: tlsCfg},
//	)
//	if err != nil {
//	    log.Fatal(err)
//	}
...
//	//   close(sess.Write)
//	<-sess.Done
func SubscribeYokeActionClient(baseURL string, opts *SubscribeYokeActionClientOptions) (*SubscribeYokeActionClientSession, error) {
	...
	u.Path = SubscribeYokeActionMeta().URL
	...
	session := &SubscribeYokeActionClientSession{
		Socket: c,
	...
	go func() {
		for {
			_, data, err := c.ReadMessage()
			session.Read <- SubscribeYokeActionReadChan{
				Data:  data,
				Error: err,
			}
			if err != nil {
