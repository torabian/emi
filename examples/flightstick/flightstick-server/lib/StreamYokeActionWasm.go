...
// StreamYokeActionReactiveHandlerWasm is the in-browser counterpart of
// StreamYokeActionReactiveHandler. It registers the same developer factory against an
...
//
func StreamYokeActionReactiveHandlerWasm(
	reactor *emigo.WasmReactor,
	factory func(session StreamYokeActionSession) (chan []byte, error),
) {
	reactor.Handle(StreamYokeActionMeta().URL, func(conn *emigo.WasmReactiveConn) error {
		session := StreamYokeActionSession{
			Done:        conn.Done,
			Read:        make(chan StreamYokeActionReadChan),
			QueryParams: StreamYokeActionQueryFromString(conn.Query.Encode()),
		...
		go func() {
			for {
				...
				case data, ok := <-conn.Read:
					...
					session.Read <- StreamYokeActionReadChan{Data: data}
				case <-conn.Done:
					return
				}
			}
