...
// SubscribeYokeActionReactiveHandlerWasm is the in-browser counterpart of
// SubscribeYokeActionReactiveHandler. It registers the same developer factory against an
...
//
func SubscribeYokeActionReactiveHandlerWasm(
	reactor *emigo.WasmReactor,
	factory func(session SubscribeYokeActionSession) (chan []byte, error),
) {
	reactor.Handle(SubscribeYokeActionMeta().URL, func(conn *emigo.WasmReactiveConn) error {
		session := SubscribeYokeActionSession{
			Done:        conn.Done,
			Read:        make(chan SubscribeYokeActionReadChan),
			QueryParams: SubscribeYokeActionQueryFromString(conn.Query.Encode()),
		...
		go func() {
			for {
				...
				case data, ok := <-conn.Read:
					...
					session.Read <- SubscribeYokeActionReadChan{Data: data}
				case <-conn.Done:
					return
				}
			}
