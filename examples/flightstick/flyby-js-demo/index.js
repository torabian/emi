...
import { SubscribeYokeAction } from "./js-sdk/SubscribeYokeAction.js";
...
/**
 * @typedef {import("./js-sdk/YokeMessageDto.js").YokeMessageDto} YokeMessageDto
 */

/**
 * @param {{ data: YokeMessageDto }} event
 */
SubscribeYokeAction.Create().onmessage = (event) => {
  console.log(111, event.data);
  flightStick.pitch = event.data.y;
  flightStick.roll = event.data.x;
};
...
const determineMovement = () => {
  ...
  if (activeKeysPressed.includes(38) || flightStick.pitch > 0) {
    // up arrow
  ...
  if (activeKeysPressed.includes(40) || flightStick.pitch < 0) {
    // down arrow
  ...
  if (activeKeysPressed.includes(37) || flightStick.roll > 0) {
    // left arrow
  ...
  if (activeKeysPressed.includes(39) || flightStick.roll < 0) {
    // right arrow
    character.rotateY(-charRotateYIncrement);
    if (charRotateYIncrement < charRotateYMax) charRotateYIncrement += 0.0005;
    if (charNeck.rotation.y < 0.7) charNeck.rotation.y += 0.07;
