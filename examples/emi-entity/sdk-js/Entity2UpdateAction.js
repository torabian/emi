import { Entity2UpdateDto } from "./Entity2UpdateDto";
...
export class Entity2UpdateAction {
  ...
  static Fetch = async (
    ...
    { creatorFn, qs, ctx, onMessage, overrideUrl } = {
      creatorFn: (item) => new Entity2UpdateActionRes(item),
    },
  ) => {
    creatorFn = creatorFn || ((item) => new Entity2UpdateActionRes(item));
 ...
 **/
export class Entity2UpdateActionRes {
  /**
   *
   * @type {string}
   **/
  ...
  /**
   * Creates an instance of Entity2UpdateActionRes, and possibleDtoObject
   * needs to satisfy the type requirement fully, otherwise typescript compile would
   * be complaining.
   **/
  static from(possibleDtoObject) {
    return new Entity2UpdateActionRes(possibleDtoObject);
  }
  /**
   * Creates an instance of Entity2UpdateActionRes, and partialDtoObject
   * needs to satisfy the type, but partially, and rest of the content would
   * be constructed according to data types and nullability.
   **/
  static with(partialDtoObject) {
    return new Entity2UpdateActionRes(partialDtoObject);
  }
  copyWith(partial) {
    return new Entity2UpdateActionRes({ ...this.toJSON(), ...partial });
  }
  clone() {
    return new Entity2UpdateActionRes(this.toJSON());
  }
}
