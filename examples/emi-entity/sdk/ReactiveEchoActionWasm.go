...
// ReactiveEchoActionReactiveHandlerWasm is the in-browser counterpart of
// ReactiveEchoActionReactiveHandler. It registers the same developer factory against an
...
//
func ReactiveEchoActionReactiveHandlerWasm(
	reactor *emigo.WasmReactor,
	factory func(session ReactiveEchoActionSession) (chan []byte, error),
) {
	reactor.Handle(ReactiveEchoActionMeta().URL, func(conn *emigo.WasmReactiveConn) error {
		session := ReactiveEchoActionSession{
			Done:        conn.Done,
			Read:        make(chan ReactiveEchoActionReadChan),
			QueryParams: ReactiveEchoActionQueryFromString(conn.Query.Encode()),
		...
		go func() {
			for {
				...
				case data, ok := <-conn.Read:
					...
					session.Read <- ReactiveEchoActionReadChan{Data: data}
				case <-conn.Done:
					return
				}
			}
