...
export class ModifyGiantDtoAction {
  ...
  static NewUrl = (qs) => buildUrl(ModifyGiantDtoAction.URL, undefined, qs);
  static Method = "post";
  static Fetch$ = async (qs, ctx, init, overrideUrl) => {
    return fetchx(
      overrideUrl ?? ModifyGiantDtoAction.NewUrl(qs),
      {
  ...
  static Fetch = async (
    ...
    const res = await ModifyGiantDtoAction.Fetch$(qs, ctx, init, overrideUrl);
