...
/**
 * Action to communicate with the action InvoiceLineGetAction
 */
data class InvoiceLineGetActionMeta(
    val name: String = "InvoiceLineGetAction",
    val url: String = "/invoiceLine/:uniqueId",
    val method: String = "get"
)
/*data class InvoiceLineGetActionRequest(val call: io.ktor.server.application.ApplicationCall)*/
data class InvoiceLineGetActionResponse(
	...
	/**
 * Path parameters for InvoiceLineGetAction
 */
data class InvoiceLineGetActionPathParameter (
	var UniqueId: String,
)
// Converts a placeholder url, and applies the parameters to it.
fun InvoiceLineGetActionPathParameterApply(params: InvoiceLineGetActionPathParameter, templateUrl: String): String {
	var url = templateUrl
		url = url.replace(":uniqueId", params.UniqueId)
	return url
}
object InvoiceLineGetActionClient {
    ...
    suspend fun compute(
		path: InvoiceLineGetActionPathParameter,
		query: Map<String, String> = emptyMap(),
		headers: Map<String, String> = emptyMap()
	): InvoiceLineGetActionResponse =
        withContext(Dispatchers.IO) {
            val meta = InvoiceLineGetActionMeta()
            var baseUrl = context?.baseUrl ?: ""
            var url = buildUrl(baseUrl, meta.url, query)
            	url = InvoiceLineGetActionPathParameterApply(path, url)
