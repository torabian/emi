...
/**
 * Action to communicate with the action TeamBrowseAction
 */
data class TeamBrowseActionMeta(
    val name: String = "TeamBrowseAction",
    val url: String = "/team/browse",
    val method: String = "get"
)
/*data class TeamBrowseActionRequest(val call: io.ktor.server.application.ApplicationCall)*/
data class TeamBrowseActionResponse(
    val statusCode: Int = 200,
    val headers: Map<String, String> = emptyMap(),
    val rawBody: String? = null,
    val payload: GResponse<TeamOptionalDto>? = null
)
object TeamBrowseActionClient {
    ...
    // into GResponse<TeamOptionalDto> once the call returns -
    // both ends are real, not stubbed.
    suspend fun compute(
		query: Map<String, String> = emptyMap(),
		headers: Map<String, String> = emptyMap()
	): TeamBrowseActionResponse =
        withContext(Dispatchers.IO) {
            val meta = TeamBrowseActionMeta()
            ...
            client.newCall(requestBuilder.build()).execute().use { resp ->
                ...
                val parsedPayload: GResponse<TeamOptionalDto>? = rawBody?.let {
                    if (it.isEmpty()) null else Json.decodeFromString<GResponse<TeamOptionalDto>>(it)
                }
                TeamBrowseActionResponse(
