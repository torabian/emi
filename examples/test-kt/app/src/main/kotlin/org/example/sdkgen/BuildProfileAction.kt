...
/**
 * Action to communicate with the action BuildProfileAction
 */
data class BuildProfileActionMeta(
    val name: String = "BuildProfileAction",
    val url: String = "/profile/build",
    val method: String = "post"
)
/*data class BuildProfileActionRequest(val call: io.ktor.server.application.ApplicationCall)*/
data class BuildProfileActionResponse(
    val statusCode: Int = 200,
    val headers: Map<String, String> = emptyMap(),
    val rawBody: String? = null,
    val payload: ProfileDto? = null
)
object BuildProfileActionClient {
        ...
        withContext(Dispatchers.IO) {
            ...
            client.newCall(requestBuilder.build()).execute().use { resp ->
                ...
                val parsedPayload: ProfileDto? = rawBody?.let {
                    if (it.isEmpty()) null else Json.decodeFromString<ProfileDto>(it)
                }
                BuildProfileActionResponse(
