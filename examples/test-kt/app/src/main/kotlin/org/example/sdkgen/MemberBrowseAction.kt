...
/**
 * Action to communicate with the action MemberBrowseAction
 */
data class MemberBrowseActionMeta(
    val name: String = "MemberBrowseAction",
    val url: String = "/member/browse",
    val method: String = "get"
)
/*data class MemberBrowseActionRequest(val call: io.ktor.server.application.ApplicationCall)*/
data class MemberBrowseActionResponse(
    val statusCode: Int = 200,
    val headers: Map<String, String> = emptyMap(),
    val rawBody: String? = null,
    val payload: GResponse<MemberOptionalDto>? = null
)
object MemberBrowseActionClient {
    ...
    // into GResponse<MemberOptionalDto> once the call returns -
    // both ends are real, not stubbed.
    suspend fun compute(
		query: Map<String, String> = emptyMap(),
		headers: Map<String, String> = emptyMap()
	): MemberBrowseActionResponse =
        withContext(Dispatchers.IO) {
            val meta = MemberBrowseActionMeta()
            ...
            client.newCall(requestBuilder.build()).execute().use { resp ->
                ...
                val parsedPayload: GResponse<MemberOptionalDto>? = rawBody?.let {
                    if (it.isEmpty()) null else Json.decodeFromString<GResponse<MemberOptionalDto>>(it)
                }
                MemberBrowseActionResponse(
