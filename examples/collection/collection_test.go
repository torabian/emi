...
// Tag is a tiny item type used inside Collection[Tag] for these tests.
type Tag struct {
	Key   string `json:"key"`
	Value string `json:"value"`
}

// Profile is a struct that embeds a Collection field — this is the shape
// emi-generated DTOs use in practice, so we want to make sure JSON round-trips
// through a real struct (not just a bare Collection) behave correctly.
type Profile struct {
	Name string                `json:"name"`
	Tags emigo.Collection[Tag] `json:"tags"`
}
...
func describe(t *testing.T, c emigo.Collection[Tag]) {
	t.Helper()
...
func TestUnmarshal_FieldOmitted(t *testing.T) {
	banner(t, "JSON has no `tags` key at all → Collection stays unset")
...
func TestMarshal_UnsetEmitsNull(t *testing.T) {
	banner(t, "Unset Collection (zero value) marshals as `null`")
	...
	fmt.Println("    ✓ zero-value Collection → null on the wire")
	fmt.Println()
}

func TestMarshal_ReplaceWithItems(t *testing.T) {
	banner(t, "CollectionReplace(items) marshals as tagged {operation:\"replace\", items:[...]}")

	p := Profile{
		Name: "ali",
		Tags: emigo.CollectionReplace([]Tag{{Key: "plan", Value: "pro"}}),
	}
	data, err := json.Marshal(p)
	if err != nil {
		t.Fatalf("marshal: %v", err)
...
func TestMarshal_AppendWithItems(t *testing.T) {
	banner(t, "CollectionAppend(items) marshals as tagged {__operation:\"append\", items:[...]}")

	p := Profile{
		Name: "ali",
		Tags: emigo.CollectionAppend([]Tag{{Key: "role", Value: "admin"}}),
	}
	data, err := json.Marshal(p)
	if err != nil {
		t.Fatalf("marshal: %v", err)
...
func TestMarshal_EmptyReplace(t *testing.T) {
	banner(t, "CollectionReplace(nil items) is still a set value (clears the child rows)")

	p := Profile{
		Name: "ali",
		Tags: emigo.CollectionReplace[Tag](nil),
	}
	data, err := json.Marshal(p)
	if err != nil {
		t.Fatalf("marshal: %v", err)
...
func TestRoundTrip_NullPreservesIsSet(t *testing.T) {
	...
	// Note: re-marshalling a null-sourced Collection emits the tagged object
	...
	if string(data) == "null" {
		t.Fatal("documented behavior: a null-unmarshalled Collection should re-marshal as a tagged object")
	}
...
func TestRoundTrip_ValuesIn_ValuesOut(t *testing.T) {
	...
	original := Profile{
		Name: "ali",
		Tags: emigo.CollectionReplace([]Tag{{Key: "a", Value: "1"}, {Key: "b", Value: "2"}}),
	}
...
func TestAPI_SetAndReset(t *testing.T) {
	...
	if !ok || ptr == nil {
		t.Fatal("Get() should return (ptr, true) for a set Collection")
	}
...
func TestAPI_SQLValue(t *testing.T) {
	...
	var unset emigo.Collection[Tag]
	v, ok := unset.SQLValue()
	step(t, "unset → (value, ok)", fmt.Sprintf("%q, %v", v, ok))
	if ok {
		t.Fatal("unset Collection must opt out of SQL column emission")
	}

	c := emigo.CollectionReplace([]Tag{{Key: "plan", Value: "pro"}})
	v, ok = c.SQLValue()
	step(t, "set → (value, ok)", fmt.Sprintf("%s, %v", v, ok))
	if !ok {
		t.Fatal("set Collection must opt into SQL column emission")
	}
...
func TestMain(m *testing.M) {
	fmt.Println()
	fmt.Println("════════════════════════════════════════════════════════════════")
	fmt.Println("  emigo.Collection[T]  —  JSON round-trip & API tests")
	fmt.Println("════════════════════════════════════════════════════════════════")
	fmt.Println()
	code := m.Run()
	fmt.Println("════════════════════════════════════════════════════════════════")
	if code == 0 {
		fmt.Println("  ✓ all Collection cases passed")
	} else {
		fmt.Println("  ✗ at least one Collection case failed (exit", code, ")")
	}
	fmt.Println("════════════════════════════════════════════════════════════════")
	fmt.Println()
	os.Exit(code)
