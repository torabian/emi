...
export type SampleSseActionQueryOptions = Omit<
  UseQueryOptions<unknown, unknown, SampleSseActionRes, unknown[]>,
  "queryKey"
> &
...
export const useSampleSseActionQuery = (
  ...
  const result = useQuery({
    queryKey: [SampleSseAction.NewUrl(options?.qs)],
    queryFn: fn,
    ...(options || {}),
  });
  return {
...
export class SampleSseAction {
  ...
  static Fetch = async (
    ...
    const res = await SampleSseAction.Fetch$(qs, ctx, init, overrideUrl);
...
export class SampleSseActionRes {
   ...
   **/
  toJSON(): SampleSseActionResType {
    return {
      message: this.#toPlainJSON(this.#message),
    } as SampleSseActionResType;
  }
  /**
  ...
  copyWith(
    partial: PartialDeep<SampleSseActionResType>,
  ): InstanceType<typeof SampleSseActionRes> {
