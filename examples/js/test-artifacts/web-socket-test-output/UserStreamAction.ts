...
export class UserStreamActionReq {
   ...
   **/
  toJSON(): UserStreamActionReqType {
    return {
      min: this.#toPlainJSON(this.#min),
      max: this.#toPlainJSON(this.#max),
      count: this.#toPlainJSON(this.#count),
    } as UserStreamActionReqType;
  }
  /**
...
export class UserStreamActionRes {
   ...
   *	class gets, rather than a shallow object still holding class instances.
   **/
  toJSON(): UserStreamActionResType {
    return {
      number: this.#toPlainJSON(this.#number),
    } as UserStreamActionResType;
  }
  /**
   * Resolves value into a plain, JSON-serializable value: recurses into
   * arrays, and - the case JSON.stringify's own recursion would only get to
