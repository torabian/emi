...
/**
 * Formats a Go code string using 'gofmt'.
 * If formatting fails, returns the original code.
 * @param {string} goCode - The Go code to format
 * @returns {Promise<string>} - Formatted Go code or original code on error
 */
export function formatGoCode(goCode: string) {
  return new Promise((resolve) => {
    const gofmt = spawn('gofmt');

    let formatted = '';
    let errorOutput = '';

    gofmt.stdout.on('data', (data: any) => {
      formatted += data.toString();
    });

    gofmt.stderr.on('data', (data: any) => {
      errorOutput += data.toString();
    });

    gofmt.on('close', (code: any) => {
      if (code === 0) {
      resolve(
  formatted.replace(/^(\t+)/gm, (match, tabs) => "  ".repeat(tabs.length))
);
      } else {
        console.warn(`gofmt failed, returning original code. Error: ${errorOutput}`);
        resolve(goCode);
      }
    });

    gofmt.stdin.write(goCode);
    gofmt.stdin.end();
  });
}
