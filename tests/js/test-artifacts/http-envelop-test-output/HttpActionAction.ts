	...
	static Fetch = async (
		...
		const res = await HttpActionAction.Fetch$(
			...
			return handleFetchResponse(
				res, 
				(data) => { const envelope = new GResponse<HttpActionActionRes>(data); envelope.updatePayload(new HttpActionActionRes(envelope.getPayload())); return envelope;},
				onMessage,
				init?.signal,
			);
	}
	...
	**/
	applyFromObject(data = {}) {
		const d = data as Partial<HttpActionActionRes>;
			if (d.recordNumber !== undefined) { this.recordNumber = d.recordNumber }
	}
	/**
