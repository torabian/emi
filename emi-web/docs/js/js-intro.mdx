...
`nest.js` and `react`, and `axios`, there is an added flags called for each of them to modify or append extra features.

* `--tags axios,axiosbundle` would generate the axios helpers.
...
For typescript support, just append `typescript` to the tags, for example `--tags axios,axiosbundle,react,nestjs,typescript` would generate them all, in typescript.