...
func ReadYamlFile[T any](filename string, out *T) error {
...
export const ?lang = ?content;`
	...
	template = stdstrings.ReplaceAll(template, "?content", body)
	template = stdstrings.ReplaceAll(template, "?lang", lang)
	return template
}
...
func CompileTranslation(ctx *TranslationResourceCatalog) {
	...
	allData += "\r\n export const strings = {...en, " + stdstrings.Join(prev, ",") + "};\r\n"
