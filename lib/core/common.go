...
type MicroGenContext struct {
	...
	Content string
	// Path is the file the "path" CLI flag pointed Content at, if any (empty when
	// content was supplied some other way). Threaded through to the parsed Emi's
	// SourcePath so preprocessing can resolve relative paths (e.g. EmiComplex.Include)
	// against it.
	Path string
}
...
func StringToEmi(content string) (Emi, error) {
	return StringToEmiWithPath(content, "")
}

// StringToEmiWithPath is StringToEmi, but also records path (the file content was
// read from, if any - pass "" when unknown) as the parsed module's SourcePath before
// preprocessing, so hooks that resolve relative paths against the declaring file (e.g.
// EmiComplex.Include) can find it. path is resolved to an absolute path first (an
...
func ReadEmiFromString(content string) (*Emi, error) {
	return ReadEmiFromStringWithPath(content, "")
}

// ReadEmiFromStringWithPath is ReadEmiFromString, but also records path as the parsed
// module's SourcePath before preprocessing - see StringToEmiWithPath.
func ReadEmiFromStringWithPath(content string, path string) (*Emi, error) {
	module, err := StringToEmiWithPath(content, path)
	if err != nil {
		return &module, err
	}
	return &module, nil
...
// ReadEmiFromFile reads path off disk and parses it exactly like ReadEmiFromString,
// additionally recording its (absolute) path as the module's SourcePath so
// preprocessing can resolve relative paths (e.g. EmiComplex.Include) against it.
func ReadEmiFromFile(path string) (*Emi, error) {
	content, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}
	return ReadEmiFromStringWithPath(string(content), path)
}
