...
func DetectIfEmiGoIsUsed2(fields []*core.EmiField) bool {
	...
	walk = func(f []*core.EmiField) {
		for _, field := range f {
			...
			if slices.Contains(
				...
				field.Type,
			) || strings.Contains(string(field.Type), "?") {
				result = true
				break
			}
			if len(field.Fields) > 0 {
...
type prepareStructInfo struct {
	EmiLocation     string
	Res             *core.CodeChunkCompiled
	RenderedClasses []goRenderedStruct
}

func PrepareStruct(fields []*core.EmiField, ctx core.MicroGenContext, goctx GoCommonStructContext) prepareStructInfo {
